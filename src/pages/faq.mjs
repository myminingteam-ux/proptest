...
const body=`<div class="wrap">${breadcrumbs([{name:'Home',path:'/'},{name:'FAQ',path:'/faq/'}])}</div>${pageHero({eyebrow:'FAQ',title:'The current rules, in plain language',lead:'Answers are aligned to the live PropFunded Rules, FAQ and Terms. The Terms remain the legal authority.'})}<section class="section"><div class="wrap">${lastReviewed('Product, Support & Legal')}<div class="faq-categories">${groups.map(group=>`<section><h2>${group}</h2><div class="faq-list" data-faq>${faqs.filter(x=>x[0]===group).map(([,q,a])=>`<article><button type="button" aria-expanded="false">${q}<span>+</span></button><div class="faq-answer"><p>${a}</p></div></article>`).join('')}</div></section>`).join('')}</div></div></section>${ctaBand()}`;
